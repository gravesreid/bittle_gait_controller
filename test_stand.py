...
prev_e = 0
num_timesteps = 1000
angle_holder = np.zeros(num_timesteps)
reference_holder = np.zeros(num_timesteps)
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    for i in range(num_timesteps):
        ...
        current_angle = data.qpos[joint_to_qpos["left-back-shoulder-joint"]]
        desired_angle = np.deg2rad(wkf[index][3])
        angle_holder[i] = current_angle
        reference_holder[i] = desired_angle
        prev_e = e
        e = -current_angle + desired_angle
        ctrl = kp*e + ki*e*dt + kd*((e-prev_e))/dt

        data.ctrl[actuator_to_ctrl["left-back-shoulder-joint"]] = np.clip(ctrl, -1, 1)
    ...
    plt.plot(time_array, angle_holder, label='Actual Trajectory')
    plt.plot(time_array, reference_holder, label='Reference Trajectory')
    plt.xlabel('Time (s)')
    plt.ylabel('Angle (rad)')
    plt.title('Reference vs Actual Trajectory for Left Back Shoulder Joint')
    plt.legend()
    plt.grid(True)
    plt.show()