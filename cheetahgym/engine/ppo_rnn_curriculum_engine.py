...
class PPORNNCurriculumEngine(PPORNNEngine):
    def __init__(self, agent, runner):
        super().__init__(agent=agent, runner=runner)

        self.terrain_parameters = {"min_gap_width": 8, "max_gap_width": 12, "min_gap_dist": 0.8, "max_gap_dist": 1.5}
    ...
    def train(self):
        for iter_t in count():
            ...
            self.adapt_parameters(train_log_info, eval_log_info)

    def adapt_parameters(self, train_log_info, eval_log_info):

        DISTANCE_THRESHOLD = 4.0 #meters
        mean_episode_forward_distance = train_log_info["train/rollout_final/forward_distance/mean"]

        if mean_episode_forward_distance > DISTANCE_THRESHOLD and self.terrain_parameters["max_gap_width"] < 50:
            print("Max Width :", self.terrain_parameters["max_gap_width"])
            self.terrain_parameters["min_gap_width"] += 2
            self.terrain_parameters["max_gap_width"] += 4
