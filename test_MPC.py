...
# MPC Parameters
N = 10          # Prediction/control horizon
mpc_dt = 0.1    # MPC timestep (100ms)
sim_dt = 0.001  # Simulation timestep (1ms)
# Calculate how many simulation steps per MPC step
...
with mujoco.viewer.launch_passive(pid_controller.model, pid_controller.data) as viewer:
    ...
    mpc_iterations = total_sim_steps // (N * steps_per_mpc_step)
    for _ in range(total_sim_steps):
        current_time = _ * dt
        for mpc_iter in range(mpc_iterations):
            ...
            # Get reference window for entire horizon
            start_idx = (mpc_iter * N) % len(reference_traj)
            ref_window = reference_traj[start_idx:start_idx+N]
            ...
            # Set references
            # Modify the state reference generation line:
            ...
            # Solve MPC for entire horizon
            sol = opti.solve()
            u_opt = sol.value(U)  # Shape (8, N)
            # Execute entire horizon
            ...
            dt = 0.001
            for k in range(N):
                # Convert to degrees and create behavior matrix
                behavior = np.rad2deg(u_opt[:,k]).reshape(1, -1)
                
                # Apply control for 'steps_per_mpc_step' simulation steps
                pid_controller.execute(
                    behavior, 
                    steps_per_mpc_step, dt,
                    kp, ki, kd,
                    viewer=viewer,
                    plotty=False
                )
